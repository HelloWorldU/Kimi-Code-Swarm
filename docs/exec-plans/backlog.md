...
- **方案 B（skill 注入 + 最小兜底）**：已经写好的 [`.kimi/skills/resolve-conflict/SKILL.md`](../../.kimi/skills/resolve-conflict/SKILL.md) 作为 prompt prefix 注入。在 `sendInstruction` 恢复 completed agent 时：① 引擎先 `git fetch origin` + `git rev-list --count HEAD..origin/main`（仅最小代码兜底）→ ② 检测 main 真的有新 commits 才尝试 `git merge origin/main` → ③ merge 干净则引擎自动 `git commit`；有冲突则将冲突文件内容 + resolve-conflict skill 作为 prefix 注入 instruction → kimi 只改冲突文件（不跑 git）→ ④ kimi 完成后引擎执行 sanity check 并 `git commit` 完成 merge。没新 commits 跳过节省 token。
...
方案 B（skill 注入 + 最小代码兜底）落地。`syncBranchWithMain` 在 `sendInstruction` 入口跑：fetch → behindCount → merge → 干净自动 commit / 冲突走 `runInstructionSilent` 让 kimi **静默**改文件 → engine 4 项 sanity check（冲突标记残留 / 文件非空 / 行数膨胀 3 倍 / git diff --check）→ 只 stage 冲突文件 commit。

**后续两轮修复**（紧跟 PR #24，解决初版的 2 个潜在问题）：
1. **conflict 解决走 silent 不污染 UI**：原版用 inline 拼 instruction 的方式注入 conflict prefix，被 `log('input', instruction)` 当用户消息显示，污染「任务指令」区和聊天面板。改成在 `syncBranchWithMain` 内部独立调 `runInstructionSilent` 完成冲突解决 → engine sanity check + commit → 然后 sendInstruction 才用原 instruction 跑用户任务。`finalizeMergeCommit` 改为纯函数式接收参数（删 `pendingMergeCommit` / `conflictFiles` / `conflictFileOriginals` 实例字段）。
2. **触发条件不再依赖 `prStatus === 'merged'`**：用户可能在 GitHub web 直接手动 merge（绕开 swarm），prStatus 永远不变 merged。改为每次 sendInstruction 入口都 fetch + behindCount 检查，5 分钟 throttle 防过度调用。

附带修复（同一轮）：
- `runInstructionSilent` 默认 idle timeout 120s → 600s（kimi 分批输出，60-180s 间隙正常，太短易误判卡死）
- `generateCommitAndPrBody` 调用 60s → 300s
- `canMerge` 自审 + fallback 分支过滤 `failed` reviewer（reviewer 跑不起来 ≠ 内容被拒，不该阻塞合并）
