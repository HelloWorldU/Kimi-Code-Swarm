...
- `components/AgentDetail.vue` — Agent 详情：`<textarea>` 指令输入（Enter 发送 / Shift+Enter 换行，自动增高）+ 日志流（含 think / tool_call / mcp / tool_result 可折叠气泡，默认收起，点击展开查看完整内容，智能滚动：仅在用户位于底部 50px 内时平滑跟进）+ PR 审阅 + 文件变更（点击通过 engine 获取 diff）。输入框草稿通过 `useSwarmStore.draftInputs` 做内存级缓存，切视图/切 Agent 时不丢失未发送内容
...
| 实时日志流 | ✅ 真实 | spawn_process + agent-stream 事件推送（支持 text / think / tool_call / mcp / tool_result 分片）；智能滚动，仅在用户位于底部 50px 内时平滑跟进 |
