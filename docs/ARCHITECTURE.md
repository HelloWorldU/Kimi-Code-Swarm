...
5. `startAgentEngine()` — 启动 Agent 引擎（失败时通过 Logger 记录 + Toast 提示用户具体错误）
...
Rust `spawn_agent_engine()` 需要找到 `agent-engine` 目录。`tauri.conf.json` 已通过 `bundle.resources` 将 `agent-engine` 打包进安装包，安装后位于 `resource_dir()/agent-engine`。
...
fn agent_engine_dir(app: &tauri::AppHandle) -> Result<PathBuf, String> {
    // 1. dev: app_local_data_dir/../../agent-engine
    // 2. dev fallback: ../agent-engine (cwd relative)
    // 3. prod: resource_dir()/agent-engine (Tauri bundle resources)
    // 4. prod fallback: exe_dir/agent-engine (portable)
    // 5. prod fallback: app_local_data_dir/agent-engine
}
```
