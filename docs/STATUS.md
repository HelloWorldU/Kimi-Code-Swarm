...
| 全员审阅门控 | ✅ | 后端严格检查 reviews 全 approved；Tauri 模式下 reviewer Agent 自动运行 kimi CLI 审阅代码变更（解析 LGTM 关键词），全部 approved 后自动合并；浏览器模式下需指挥官手动审阅 | `src/store/useSwarmStore.ts`, `agent-engine/src/agent.ts`, `agent-engine/src/engine.ts` |
