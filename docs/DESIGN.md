...
当前包含三个模板：
- **`harness/new-instance.yaml`**：新建 Agent 的完整流程（收集信息 → 创建 Agent → 启动 Engine → 多轮对话 → **自动提交审阅**（检测到代码变更后自动创建 PR，pre-commit 失败时自动修复重试）→ **自动合并**（全员审阅通过后 Engine 自动调用 GitHub API 合并 PR））
