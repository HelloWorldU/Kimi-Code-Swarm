...
| `error-handling.ts` | `error-handling/empty-catch` | catch 块禁止为空（红线：错误禁止静默吞没），支持 `catch (err) {` / `catch {` / `catch (err: unknown) {` 等全部变体，`--fix` 自动注入 `log.error` | ✅ |
| | `error-handling/missing-logger` | catch 块建议记录错误（warn），识别形式包括 `log.error(...)` / `this.log('error', ...)` / `console.error(...)` / `throw new Error(...)`；warn 级别不阻断提交，仅提示 | ⚡ |
