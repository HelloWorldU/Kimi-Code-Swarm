...
| `error-handling.ts` | `error-handling/empty-catch` | catch 块禁止为空（红线：错误禁止静默吞没），但**有明确意图注释的不算空**（如 `// expected: ...` / `// 正常噪音: ...`）。基于 ESTree AST 解析，自动覆盖全部语法变体（含 ES2019 `catch {`），天然过滤字符串/注释误报，`--fix` 自动注入 `log.error` | ✅ |
| | `error-handling/missing-logger` | catch 块建议记录错误（warn）。关键路径必须有 `log.error` / `console.error`；非关键路径可用**意图注释**替代（如 `// expected: 目录不存在`）。敷衍注释（`// ignore` / `// TODO`）不算。warn 级别不阻断 | ⚡ |

### 3. CI 流水线 (`npm run ci`)

```
typecheck → lint → analyze → test → check-docs → check-test-sync → build
```
...
3. **执行即更新文档** — 代码变更后必须同步更新相关文档；被 check-docs 阻断时回顾本次会话查阅过的文档
4. **功能状态必须披露** — 实现后须在 [`docs/STATUS.md`](STATUS.md) 标记状态
5. **约束即代码** — 所有规则必须机械可执行
6. **代码改动必验证** — 改完后必须实际 build、启动 app、运行测试、过 lint/analyze，全部通过后才允许合入
7. **debug 必加日志** — 代码看不出根因时，禁止盲猜。立即增加日志（`src/utils/logger.ts`）定位；修复后在注释或 commit 中记录根因
...
当前 `v0.2.1`（pull SKILL + SKILL 格式统一）。版本号 bump 需同步 `kimi-code-swarm/package.json` / `tauri.conf.json` / `Cargo.toml` 三处，并新建 git tag `vX.Y.Z`。
