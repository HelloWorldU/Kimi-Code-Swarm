...
| `error-handling.ts` | `error-handling/empty-catch` | catch 块禁止为空（错误禁止静默吞没） | ✅ |
| | `error-handling/missing-logger` | catch 块必须使用 Logger 记录错误（harness/bug-fix.yaml instrument 硬化） | ✅ |
