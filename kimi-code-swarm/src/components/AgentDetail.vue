...
const emit = defineEmits<{
  ...
  (e: 'submitReview', agentId: string, reviewerAgentId: string, approved: boolean): void
...
const statusColor = computed(() => {
  if (props.agent.status === 'working') return 'text-amber-400'
  if (props.agent.status === 'completed') return 'text-emerald-400'
  if (props.agent.status === 'stopped') return 'text-red-400'
  if (props.agent.status === 'reviewing') return 'text-purple-400'
  ...
  <div class="h-full flex flex-col">
    ...
    <div class="flex items-center gap-4 mb-6">
      ...
      <div class="flex-1 min-w-0">
        <h2 class="text-xl font-bold text-white truncate">{{ agent.name }}</h2>
      ...
      <div class="text-right shrink-0">
        <p class="text-sm font-medium text-gray-300">{{ agent.tokenUsed.toLocaleString() }} <span class="text-gray-500">/ {{ agent.tokenBudget.toLocaleString() }}</span></p>
    ...
    <div class="grid grid-cols-2 gap-3 mb-4">
      <div class="px-4 py-3 rounded-lg bg-gray-800/50 border border-gray-700/50">
        ...
        <p class="text-sm text-gray-200 mt-1">{{ agent.instruction || '暂无指令' }}</p>
      </div>
      <div class="px-4 py-3 rounded-lg bg-gray-800/50 border border-gray-700/50">
        <p class="text-xs text-gray-400">仓库</p>
        <p class="text-sm text-gray-200 mt-1 truncate">{{ agent.repoUrl }}</p>
      </div>
    </div>

    <!-- PR Panel -->
    <div v-if="agent.prStatus !== 'none'" class="mb-4 px-4 py-3 rounded-lg bg-purple-500/5 border border-purple-500/20 space-y-3">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
          <GitPullRequest class="w-4 h-4 text-purple-400" />
          <span class="text-sm font-medium text-purple-300">PR #{{ agent.prNumber }}</span>
          <span
            :class="[
              'text-xs px-2 py-0.5 rounded-full',
              agent.prStatus === 'open' ? 'bg-purple-500/10 text-purple-400' :
              agent.prStatus === 'merged' ? 'bg-emerald-500/10 text-emerald-400' :
      ...
      <div v-if="agent.reviews.length > 0 && agent.status === 'reviewing'">
        <div class="flex items-center justify-between text-xs mb-1.5">
          ...
          <span :class="approvedCount === agent.reviews.length ? 'text-emerald-400' : 'text-gray-300'">
      ...
      <a v-if="agent.prUrl" :href="agent.prUrl" target="_blank" class="text-xs text-gray-500 hover:text-swarm-400 block">{{ agent.prUrl }}</a>
    </div>
    ...
    <div v-if="agent.changedFiles && agent.changedFiles.length > 0" class="mb-4">
      <div class="flex items-center gap-2 mb-2">
        ...
        <span class="text-xs font-medium text-gray-400">文件变更 ({{ agent.changedFiles.length }})</span>
      </div>
